    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
        ...
        if labels is not None:
            ...
            if attention_mask_label is not None:
                crf_layer = CRF(logits.shape[2], batch_first=True)
                loss = (- crf_layer(logits.clone().cpu(), labels.clone().cpu(), mask=attention_mask_label.clone().cpu().byte())).to("cuda")
            else:
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["O", "B-Course_number", "I-Course_number",  "B-Course_title", "I-Course_title", "B-Email", "I-Email", "B-Person", "I-Person", "B-Textbook", "I-Textbook", "[CLS]", "[SEP]"]
...
def main():
    ...
    if args.do_train:
        ...
        model_config = {"bert_model": args.bert_model, "do_lower": args.do_lower_case,
                        ...
                        "label_map": label_map}
        json.dump(model_config, open(os.path.join(args.output_dir, "model_config.json"), "w", encoding="utf-8"))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask in tqdm(eval_dataloader,
            ...
            crf_layer = CRF(logits.shape[2] - 1, batch_first=True)
            logits = crf_layer.decode(logits.clone().cpu()[:,:,1:])
            logits = [[z + 1 for z in y] for y in logits]
            logits = array(logits)
